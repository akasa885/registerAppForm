...
@section('content')
    ...
    <div class="container-fluid mx-auto p-6 @if ($link->count() == 0) h-full @endif">
        <div class="flex items-stretch -mx-4 flex-col">
            ...
            @else
                <div class="p-10 grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5"
                    id="list-wrapper">
                    @foreach ($link as $item)
                        ...
                        <div class="bg-white rounded-lg shadow-md overflow-hidden relative">
                            <div class="relative">
                                ...
                                @endphp
                                <div class="absolute top-2 right-2 {{ $styleTag }} text-xs px-2 py-1 rounded">
                                    {{ $messageTag }}</div>
                            </div>
                            <div class="p-4">
                                ...
                                @php
                                    $taglessBody = html_entity_decode(strip_tags($item->description));
                                @endphp
                                ...
                                <div class="flex items-center mt-4 space-x-4 text-sm text-gray-600">
                                    <div class="flex items-center">
                                        ...
                                        @php
                                            $dateEvent =
                                                $item->event_date != null ? $item->event_date : $item->active_until;
                                            $dateEvent = Carbon::parse($dateEvent)->translatedFormat('d F Y');
                                        @endphp
                                        <span>{{ $dateEvent }}</span>
                                    </div>
                                    {{-- <div class="flex items-center">
